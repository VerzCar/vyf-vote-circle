...
func TestWithFiles(t *testing.T) {
	...
	t.Run(
		"with one file", func(t *testing.T) {
	...
	t.Run(
		"with multiple files", func(t *testing.T) {
			tClient := NewTestClient(
				func(req *http.Request) *http.Response {
					mediaType, params, err := mime.ParseMediaType(req.Header.Get("Content-Type"))
					require.NoError(t, err)
	...
	t.Run(
		"with multiple files across multiple variables", func(t *testing.T) {
			tClient := NewTestClient(
				func(req *http.Request) *http.Response {
					mediaType, params, err := mime.ParseMediaType(req.Header.Get("Content-Type"))
					require.NoError(t, err)
	...
	t.Run(
		"with multiple files and file reuse", func(t *testing.T) {
			tClient := NewTestClient(
				func(req *http.Request) *http.Response {
					mediaType, params, err := mime.ParseMediaType(req.Header.Get("Content-Type"))
					require.NoError(t, err)
					...
					for {
						...
						require.False(
							t,
							regexp.MustCompile(`form-data; name="2"; filename=.*`).MatchString(contentDisposition),
						)
					}
					return &http.Response{
						StatusCode: 200,
			...
			c.Post(
				"{ id }", &resp,
				Var("files", []*os.File{tempFile1, tempFile2, tempFile1}),
				WithFiles(),
			)
		},
	)
